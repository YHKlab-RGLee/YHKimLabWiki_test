...
`atomic unit`은 원자 세계의 계산에 아주 유용한 `natural unit system`이다. 즉 수식에 붙는 갖가지 상수들을 단위로 옮김으로써 (dimension analysis) 수식을 단순화하여 표현 대상이 따르는 물리법칙을 파악하는 데에 도움을 준다. 또한 여러 불필요한 자릿수 계산(e.g. \(10^{-11} \times 10^{-20} \times 10^{8} \times 10^{32} \times m/sec\)) 등의 연산량을 줄이며, 코딩을 통해 수식을 작성할 때에도 용이하게 해준다.
...
where the kinetic energy is \(-\frac{\hbar^2}{2m_e}{\nabla}^2\), and the potential energy (by Coulomb force) term is \(-\frac{e^2}{4\pi \epsilon_0 r}\).
...
\(r = \lambda r'\)  
\(\lambda = \frac{4\pi \epsilon_0 \hbar^2}{m_e e^2} = a_0 = 1 \text{ Bohr} = 5.292 \times 10^{-11} \, m\)

\(E = E_a \times E'\)  
\(E_a = \frac{\hbar}{m_e a_0^2} = 2 \text{ Rydberg} = 1 \text{ Hartree}\)

이라 하면 해밀토니안에 대한 표현식은
...
길이가 \(100 \AA\) 이고 background potential이 0 \(eV\) 인 1차원 무한 양자 우물에서의 슈뢰딩거 방정식을 풀게 되면 그 Eigenstate와 Eigenvalue는 (간격은 \(a = 1 \AA\))

\[
\phi_\alpha(x) = \sin(k_\alpha x)
\]

\[
E_\alpha = \frac{\hbar^2 k_\alpha^2}{2m}, \qquad k_\alpha = \frac{\alpha \pi}{L} \quad (\alpha = 1, 2, 3, \ldots)
\]
...
\[
\left( \frac{\partial^2 \psi}{\partial x^2} \right)_{x=x_n}
= \frac{1}{a^2} \left[ \psi(x_{n+1}) - 2\psi(x_n) + \psi(x_{n-1}) \right]
\]
...
\begin{aligned}
[H_{op} \psi]_{x=x_n}
&= - \frac{\hbar^2}{2m} \left( \frac{\partial^2 \psi}{\partial x^2} \right)_{x=x_n} \\
&= t_0 \left[ 2\psi(x_n) - \psi(x_{n+1}) - \psi(x_{n-1}) \right] \\
&= \sum_m t_0 \left[ 2 \delta_{n,m} - \delta_{n+1,m} - \delta_{n-1,m} \right] \psi_m \\
...
\begin{aligned}
[H_{op} \psi]_{x'=x_n'}
&= - \frac{1}{2} \left( \frac{\partial^2 \psi}{\partial x^2} \right)_{x'=x_n'} \\
&= {t_0}' \left[ 2\psi(x_n') - \psi(x_{n+1}') - \psi(x_{n-1}') \right] \\
&= \sum_m {t_0}' \left[ 2 \delta_{n,m} - \delta_{n+1,m} - \delta_{n-1,m} \right] \psi_m
\end{aligned}
\]
